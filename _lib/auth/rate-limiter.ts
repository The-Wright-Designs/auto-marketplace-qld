...
/**
 * Check rate limit for a specific action and identifier
 */
...
export async function checkIpRateLimit(
  ...
  // Use stricter limits for IP-based checks
  const ipKey = `ip:${action}:${ip}`;
  const now = Date.now();

  // Stricter config for IP-based limiting
  const ipConfig = {
  ...
  if (!current || now > current.resetTime) {
    ...
    rateLimitStore.set(ipKey, newData);
  ...
  rateLimitStore.set(ipKey, updatedData);
