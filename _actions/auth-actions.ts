...
export async function refreshSessionAction(): Promise<AuthResult> {
  try {
    ...
    // Create new session cookie using the custom token
    // Note: createSessionCookie expects an ID token, but we're using custom token
    // This is a workaround since we don't have a fresh ID token in server context
